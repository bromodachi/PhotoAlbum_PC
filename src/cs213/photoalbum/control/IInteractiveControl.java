...
public interface IInteractiveControl extends IErrorControl {
	 ...
	 * 
	 * @param albumId
	 *            is the name of the album and will print the pictures(if any)
	 *            information
	 */
	public void listPhotos(String albumId);

	/**
	 * adds the photo given the album id, the photo file name, and the caption
	 * the user wishes to add
	 * 
	 * @param albumId
	 *            is what the user wishes to add the photo to. If the album
	 *            doesn't exist, print an error
	 * @param photoFileName
	 *            the name of the photo. If it already exists, print an error.
	 * @param photoCaption
	 *            the caption the user wishes to add to the photo.
	 */
	 ...
	 * 
	 * @param albumIdSrc
	 *            the source where the photo currently lives. If the user inputs
	 *            an invalid name of the source, we print an error
	 * @param albumIdDest
	 *            the destination we wish to move the photo to. If the user
	 *            passes an invalid name of the destination, we print an error
	 * @param photoId
	 *            the add of the photo we will be moving. If the user passes an
	 *            id that doesn't match the id in the source album.
	 */
	 ...
	 * 
	 * @param albumId
	 *            is the album the user wishes to remove the photo from. If the
	 *            user inputs an invalid album id, we print an error
	 * @param photoId
	 *            is the photo id we wish to remove. If the user inputs an
	 *            invalid photoID we print an error. If both succeed, print an
	 *            message that to tell the user successfully removed that photo
	 *            in that album
	 */
	 ...
	 * 
	 * @param photoId
	 *            the id the user wishes to tag. If the id doesn't exist, we
	 *            print an error
	 * @param tagType
	 *            the tag type the user wishes to add. If the tag type doesn't
	 *            exist, another error to be printed
	 * @param tagValue
	 *            the value that will be attached to the photo prints a
	 *            successful message once we successfully tagged the photo
	 * @return
	 */
	 ...
	 * 
	 * @param photoId
	 *            is the photo id we wish to delete the tag off of. If it
	 *            doesn't exist, error
	 * @param tagType
	 *            is the tagtype we wish to delete. If it doesn't exist for that
	 *            said photo, error.
	 */
	public void deleteTag(String photoId, String tagType, String tagValue);

	/**
	 * prints the photo information which includes, the file name, album, date,
	 * caption, and tags
	 * 
	 * @param photoId
	 *            the photo id we wish to print. If it doesn't exist, ask IEC to
	 *            print error
	 */
	 ...
	 * 
	 * @param start
	 *            user much specify a valid date, if user passes anything else
	 *            like a string, error
	 * @param end
	 *            user much specify a valid date for the end, if user passes
	 *            anything else like a string, error end paramater should not be
	 *            before the start date.
	 */
	public void getPhotosByDate(String start, String end);

	/**
	 * retrieves all photos by the tag in chronological order. Tags can be
	 * specified with or without their types
	 * 
	 * @param tagType
	 *            the tag the user which to specify. If not, tagtype can be
	 *            empty
	 */
